 ...
 * @type {[type]}
 */
var logger = require('../utils/log4jsutil').logger(__dirname+'/comment.js');
var express = require('express');
var router = express.Router();
var ru = require('../utils/routersutil');
var httputil = require('../utils/httputil'); 
var {Type} = require('../utils/db');
var {DCommentExtend,Notice,NoticeType} = require('../utils/module');
  ...
  discussDao.queryOpDiscuss(id,userid,function(err,item){
      ...
      if(userid != item.userid){//自己点赞自己不发消息
        ...
        var title = '有人'+op_srt+'了你的讨论';
          ...
          var message = new Notice(NoticeType.DCOMMENT,item.userid,title,content,commentExtend);
          httputil.requstPSPost('/cmapp/message/actionmsg',message,function(err,result){
            logger.debug(err);
            logger.debug(result);
          }); 
          if(cid > 0 ){
            discussDao.queryCommentUserid(cid,function(err,result){
              if(err){
                logger.error(err);
              }else{
                ...
                if(comment_userid&&comment_userid != userid&&comment_userid != item.userid){// 屏蔽回复自己跟作者本身
                  ...
                  var message1 = new Notice(NoticeType.DCOMMENT,comment_userid,title,content,commentExtend);
                  httputil.requstPSPost('/cmapp/message/actionmsg',message1,function(err,result){
                    logger.debug(err);
                    logger.debug(result);
                  });
                }
